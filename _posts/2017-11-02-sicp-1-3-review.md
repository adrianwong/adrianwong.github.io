...
## Exercise 1.38
For this fraction, we know that all values of N<sub>i</sub> are `1`. D<sub>i</sub> however, is slightly less straightforward. The (convoluted) pattern I derived from the D<sub>i</sub> series is `2(i + 1)/3` if `(i + 1) % 3 == 0`, for all `i > 1`.
...
## Exercise 1.39
Fairly obvious patterns in both series: N<sub>i</sub> = -x<sup>2</sup> for i > 1, and D<sub>i</sub> = 2i - 1.
...
Applying the substitution model shows us that the procedure makes 2<sup>4</sup> `inc` calls, which gives us the answer `5 + 16 = 21`.
...
From this table, we can determine that average_damps = log<sub>2</sub> (root), rounded down to the nearest integer.

Scheme does not have a `log` procedure that allows us to specify a base. To get around this, we can use the log change-of-base formula: log<sub>b</sub> n = log<sub>a</sub> n / log<sub>a</sub> b.
